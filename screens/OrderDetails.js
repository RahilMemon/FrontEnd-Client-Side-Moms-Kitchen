              ...
              <View className="bg-white shadow shadow-black px-4 py-3 mx-1 rounded-xl space-y-4">
                <View className="flex-row">
                  <View className="w-28 h-28 rounded-3xl bg-white shadow-md shadow-black">
                    <Image
                      source={{ uri: SERVER_URL + order?.order.dish.photo }}
                ...
                <View className={`${(order?.order.orderStatus !== 'Rejected' && order?.order.deliveryStatus !== 'Delivered') && 'border-t-2 border-slate-300'} -mb-2`}>
                  {isAuthenticated &&
                    user.userType === "Chef" &&

...
import { getChefDishes, getDishes } from "../redux/actions/dishAction";
...
const ChefProfile = () => {
  ...
  const [feedbacks, setFeedbacks] = useState(dishes?.Dishes.feedbacks || [])
  const [chefAvgRating, setchefAvgRating] = useState(0)
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // const dishesChefs = dishes?.Dishes.filter((item) => item.chef._id === _id)
    setDishesChefs((dishesChefs) =>
      dishesChefs.filter((item) => item.chef._id === _id)
    );

    // const totalRatings = feedbacks.reduce((acc, feedback) => acc + feedback.rating)
